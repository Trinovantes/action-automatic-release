...
const parser = __webpack_require__(1585)
const regex = __webpack_require__(5516)
...
;// CONCATENATED MODULE: ./src/utils/searchPrevReleaseTag.ts

async function searchPrevReleaseTag(ghClient, ghRepo, currentTag) {
    const validSemver = (0,semver.valid)(currentTag);
    if (!validSemver) {
    ...
    const allTags = await ghClient.paginate(listTagsOptions);
    const semverTags = allTags
        .filter((tag) => (0,semver.valid)(tag.name))
        .map((tag) => ({
        name: tag.name,
        semverName: tag.name,
    }))
        .sort((a, b) => (0,semver.rcompare)(a.semverName, b.semverName));
    for (const tag of semverTags) {
        if ((0,semver.lt)(tag.semverName, currentTag)) {
            return tag.name;
        }
    }
...
var conventional_commits_parser = __webpack_require__(960);
...
// generateChangeLog
// ----------------------------------------------------------------------------
async function generateChangeLog(client, context, prevReleaseTag, headSha) {
...
// Helpers
// ----------------------------------------------------------------------------
function getChangeLog(parsedCommits, label, filter) {
    let changeLog = '';
    const filteredCommitLogs = parsedCommits
        .filter(filter)
        .map((commit) => getFormattedChangeLogEntry(commit));
    if (filteredCommitLogs.length > 0) {
        changeLog += external_node_os_namespaceObject.EOL + external_node_os_namespaceObject.EOL;
        changeLog += external_node_os_namespaceObject.EOL + external_node_os_namespaceObject.EOL;
        changeLog += `## ${label}` + external_node_os_namespaceObject.EOL;
        changeLog += filteredCommitLogs.join(external_node_os_namespaceObject.EOL).trim();
    }
    return changeLog;
}
function getFormattedChangeLogEntry(commit) {
...
function isBreakingChange(body, footer) {
    const re = /^BREAKING\s+CHANGES?:\s+/;
    return re.test(body ?? '') || re.test(footer ?? '');
}
async function getCommitsBetweenReleases(client, context, prevRelease, currRelease) {
    try {
    ...
    try {
        ...
        for (const commit of compareResult.data.commits) {
            ...
            const parsedCommit = (0,conventional_commits_parser.sync)(commit.commit.message);
...
;// CONCATENATED MODULE: ./src/AutomaticRelease.ts
...
class AutomaticRelease {
    ...
    async createOrUpdateTag(tag, head) {
    ...
    async createRelease(tag, releaseName, changeLog) {
